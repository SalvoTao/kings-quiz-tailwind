      ...
      <div className="grid grid-cols-5 grid-rows-[0.5fr_1fr_1fr_1fr_1fr_1fr] w-full h-full gap-1 p-2">
        ...
        {points.map((point) =>
          categories.map((_, index) => (
            <div
              key={`${index}-${point}`}
              className="bg-gradient-to-b from-blue-600 to-blue-800 text-white text-center font-bold cursor-pointer border border-blue-500 
              text-xs sm:text-sm md:text-lg lg:text-xl xl:text-2xl 
              flex items-center justify-center h-full w-full 
              rounded-xl shadow-[0px_6px_0px_0px_rgba(0,0,0,0.4)] transition-all duration-300 ease-in-out 
